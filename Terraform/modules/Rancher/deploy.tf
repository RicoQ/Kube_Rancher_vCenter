...
resource "vsphere_virtual_machine" "vm" {
 ...
 provisioner "local-exec" {
    command = "ansible-playbook -u ${var.ssh_user} -i \"${self.default_ip_address},\" -e \"USER=${var.ssh_user}\" -e \"TERRA=ACTIVE\" -e \"SERVICE=Rancher\" -e \"VERSION=1.0\" -e \"ansible_password=${var.ssh_pwd}\" -e \"ansible_sudo_pass=${var.ssh_pwd}\" ../ansible/playbooks/main.yml"
    when = create
  }
}